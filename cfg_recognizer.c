// cfg_recognizer.c
// Compile: gcc -std=c99 cfg_recognizer.c -o cfg_recognizer
...
int find_nt(const char *name) {
    for (int i=0;i<n_nts;i++) if (strcmp(nts[i].name,name)==0) return i;
    return -1;
}
...
void add_production(const char *lhs, Symbol *syms, int nsyms) {
    ...
    for (int i=0;i<nsyms;i++) p->syms[i] = syms[i];
}
        ...
        *arrow = 0;
        ...
        while (alt) {
            ...
            if (strcmp(seq, "ε")==0 || strcmp(seq,"e")==0 || strcmp(seq,"") == 0) {
                ...
                for (int k=0;k<tcount;k++) arr[k]=temp[k];
...
void union_into(int *target, int *src, int n) {
    for (int i=0;i<=n;i++) if (src[i]) target[i]=1;
}
...
int *match_sequence(Symbol *syms, int nsyms, int pos, const char *inp, int n) {
    ...
    for (int k=0;k<nsyms;k++) {
        ...
        free(curr);
        curr = next;
        int any=0; for (int i=0;i<=n;i++) if (curr[i]) { any=1; break; }
        if (!any) break;
    }
    return curr;
}
